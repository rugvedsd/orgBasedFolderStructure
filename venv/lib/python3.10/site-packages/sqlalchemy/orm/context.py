...
class ORMSelectCompileState(ORMCompileState, SelectState):
    _joinputath = _joinputoint = _EMPTY_DICT
    ...
    def _legacy_join(self, args):
        ...
        for (right, onclause, left, flags) in args:
            ...
            from_joinputoint = flags["from_joinputoint"]
            full = flags["full"]
            ...
            if not from_joinputoint:
                self._reset_joinputoint()
            else:
                prev_aliased_generation = self._joinputoint.get(
            ...
            if isinstance(onclause, util.string_types):
                ...
                onclause = _entity_namespace_key(
                    inspect(self._joinputoint_zero()), onclause
                )

            # legacy vvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
            # check for q.join(Class.propname, from_joinputoint=True)
            # and Class corresponds at the mapper level to the current
            # joinputoint.  this match intentionally looks for a non-aliased
            # class-bound descriptor as the onclause and if it matches the
            # current joinputoint at the mapper level, it's used.  This
            ...
            elif from_joinputoint and isinstance(
                onclause, interfaces.PropComparator
            ):
                jp0 = self._joinputoint_zero()
                info = inspect(jp0)
    ...
    def _joinputoint_zero(self):
        return self._joinputoint.get("_joinputoint_entity", self._entity_zero())
    ...
    def _join_check_and_adapt_right_side(
        ...
        if not create_aliases and prop:
            self._update_joinputoint(
                {
                    "_joinputoint_entity": right,
                    "prev": ((left, right, prop.key), self._joinputoint),
    ...
    def _update_joinputoint(self, jp):
        self._joinputoint = jp
        # copy backwards to the root of the _joinputath
    ...
    def _reset_joinputoint(self):
        self._joinputoint = self._joinputath
